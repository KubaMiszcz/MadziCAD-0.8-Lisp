(defun c:madzicad_distance (/ ANGLEDEG DELTA_X DELTA_Y DELTA_Z DIST DISTCOORDS MESSAGE PK1 PK2)
  ...
  (setq	dist (sqrt
	       (+ (expt (car distcoords) 2) (expt (cadr distcoords) 2))
	     )
  )
  ...
  ;(startapp
  ;  (strcat path "\\support\\madzicad_message_box.exe")
  ;)
  (alert message)
  (princ)
)